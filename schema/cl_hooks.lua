...
local empireOverlay = ix.util.GetMaterial("effects/combine_binocoverlay")
...
function Schema:RenderScreenspaceEffects()
	...
	if (LocalPlayer():IsEmpire()) then
		...
		empireOverlay:SetInt("$ignorez", 1)

		render.SetMaterial(empireOverlay)
...
function Schema:CreateCharacterInfo(panel)
	if (LocalPlayer():Team() == FACTION_STORM) then
...
netstream.Hook("EmpireDisplayMessage", function(text, color, arguments)
	if (IsValid(ix.gui.empire)) then
		ix.gui.empire:AddLine(text, color, nil, unpack(arguments))
	end
end)
...
netstream.Hook("ViewObjectives", function(data)
	Schema:AddEmpireDisplayMessage("@cViewObjectives")
